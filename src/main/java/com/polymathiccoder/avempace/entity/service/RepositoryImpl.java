...
// Life cycle
	public RepositoryImpl(final DynamoDBDMLOperationsService dynamoDBDMLOperationsService, final Class<T> pojoType, final Region region) {
		this.dynamoDBDMLOperationsService = dynamoDBDMLOperationsService;
		this.dynamoDBDMLOperationsService.useRegion(region);
...
// Behavior
	...
	public T find(final EntityPropertyValueCriteria entityPropertyValueCriteria) {
		...
		final Entity<T> entity = tuple.toEntity(pojoType);

		return entity.getPojo();
	}

	@Override
	public List<T> findAllBy(final EntityPropertyValueCriteria entityPropertyValueCriteria) {
		final TableDefinition tableDefinition = mapping.getTableDefinition();
		...
		for (final EntityPropertyValueCriterion entityPropertyValueCriterion : entityPropertyValueCriteria.get()) {
			final AttributeSchema attributeSchema = mapping.lookupByPropertyName(entityPropertyValueCriterion.getOf()).getAttributeSchema();
	...
	public List<T> findAll() {
		return findAllBy(EntityPropertyValueCriteria.anything());
	}
	...
	public void update(final EntityPropertyValueCriteria entityPropertyValueCriteria, final EntityPropertyValueOperations entityPropertyValueOperations) {
		...
		for (final EntityPropertyValueOperation entityPropertyValueOperation : entityPropertyValueOperations.get()) {
			...
			final SchemaMappingEntry schemaMappingEntry = mapping.lookupByPropertyName(entityPropertyValueOperation.getOn());
	...
	// Helpers
	public Set<Attribute> constructKeyAttributes(final T pojo) {
		final TableDefinition tableDefinition = mapping.getTableDefinition();
		...
		final PropertySchema hashKeyPropertySchema =  mapping.lookupByAttributeSchema(hashKeyAttributeSchema).getPropertySchema();
		...
		if (tableDefinition.getRangeKeySchema().isPresent()) {
			...
			final PropertySchema rangeKeyPropertySchema =  mapping.lookupByAttributeSchema(rangeKeyAttributeSchema).getPropertySchema();
...
/////// VAlidate for no mapping
	...
	private Set<Attribute> inferAttributes(final Set<EntityPropertyValueCriterion> entityPropertyValueCriteria) {
		...
		for (final EntityPropertyValueCriterion pojoPropertyValueCriterion : entityPropertyValueCriteria) {
			final AttributeSchema attributeSchema = mapping.lookupByPropertyName(pojoPropertyValueCriterion.getOf()).getAttributeSchema();
